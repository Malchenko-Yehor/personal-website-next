...
export const getExperience = (dateRangesList: Array<DateRange>) => {
  ...
  const yearsOfExperience = expAmountInYears !== 0
    ? timeFormat.format(expAmountInYears, 'year').match(/\d+.+/g)[0]
    : '';
  const monthsOfExperience = expAmountInMonths !== 0
    ? timeFormat.format(expAmountInMonths, 'month').match(/\d+.+/g)[0]
    : '';
